...
The initial round is the exhaustive pass: the prompt requires a per-file, all-severity sweep (bug, risk, **and** nit), and each coverage entry claims a completed sweep of that file. It is deliberately recall-first because a fixing agent adjudicates the batch. Demonstrated failures are `bug`s; credible material failure paths with a condition or proof gap are `risk`s; objective low-impact defects are `nit`s. Evidence-equivalent drafts at the same primary code location are consolidated so the agent does not adjudicate the same issue twice, while separate locations remain separately tracked. From verify round 2 onward a **severity floor** applies: carried `nit` findings are retired — stated visibly in the round's resolution section, never silently — so follow-up rounds track the bug/risk backlog to convergence instead of re-adjudicating nits forever.
