...
The initial round is the exhaustive pass: the prompt requires a per-file, all-severity sweep (bug, risk, **and** nit), and each coverage entry claims a completed sweep of that file. It is deliberately recall-first because a fixing agent adjudicates the batch: an unresolved but genuine candidate stays visible as a `risk` with its proof gap, while demonstrated failures are `bug`s. Evidence-equivalent drafts with the same trigger, root cause, and fix are consolidated so the agent does not adjudicate the same issue twice. From verify round 2 onward a **severity floor** applies: carried `nit` findings are retired — stated visibly in the round's resolution section, never silently — so follow-up rounds track the bug/risk backlog to convergence instead of re-adjudicating nits forever.
